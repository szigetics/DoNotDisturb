...
-(void)toggleDaemon:(BOOL)shouldLoad reply:(void (^)(NSNumber*))reply
{
    ...
    //plist path
    NSString* plist = [@"/Library/LaunchDaemons" stringByAppendingPathComponent:LAUNCH_DAEMON_PLIST];
    
    //args
    NSArray* args = shouldLoad
        ? @[@"bootstrap", @"system", plist]
        : @[@"bootout", @"system", plist];
    
    //execute
    results = execTask(@"/bin/launchctl", args, YES, NO);
