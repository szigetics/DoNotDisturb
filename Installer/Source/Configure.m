...
-(BOOL)lsRegisterDaemon:(BOOL)shouldRegister
{
    ...
    {
        //err msg
        os_log_error(logHandle, "ERROR: failed to execute %{public}@ (%{public}@)", LSREGISTER, results);
...
-(BOOL)uninstall:(BOOL)full
{
    ...
    // can't do this in script since it needs to be executed as logged in user (not r00t)
    if(!toggleLoginItem([NSURL fileURLWithPath:loginItem], ACTION_UNINSTALL_FLAG)) {
