...
-(void)configure
{
    ...
    //init status msg
    self.statusMsg.stringValue = @"Thwart the Evil Maids 🙅🏻‍♂️";
...
-(IBAction)configureButtonHandler:(id)sender {
    ...
    switch(action)
    {
        ...
        {
            //dbg msg
            os_log_debug(logHandle, "showing 'info' view");
            
            //remove title
            self.window.title = @"";
            
            //show view
            [self showView:self.infoView firstResponder:self.infoViewNextButton];
            ...
            if(YES == [self.configureObj shouldRequestFDA])
            {
                ...
                //set tag
                self.infoViewNextButton.tag = ACTION_SHOW_FDA;
            }
            //no need
            ...
            else
            {
                ...
                //set tag
                self.infoViewNextButton.tag = ACTION_SHOW_CONFIGURATION;
            }
        ...
        case ACTION_SHOW_FDA:
        {
            ...
            ^{
                ...
                ^{
                    ...
                    //enable 'next' button
                    ((NSButton*)[self.diskAccessView viewWithTag:ACTION_SHOW_CONFIGURATION]).enabled = YES;
                    
                    //make it first responder
                    [self.window makeFirstResponder:[self.diskAccessView viewWithTag:ACTION_SHOW_CONFIGURATION]];
                });
            });
        ...
        case ACTION_SHOW_CONFIGURATION:
        {
            ...
            //show view
            [self showView:self.configureView firstResponder:[self.configureView viewWithTag:ACTION_SHOW_SUPPORT]];
        ...
        case ACTION_CLOSE_FLAG:
        {
            ...
            if(self.supportView.window.isVisible) {
...
-(void)completeEvent:(BOOL)success event:(NSInteger)event
{
        ...
        (ACTION_INSTALL_FLAG == event) )
    {
        ...
        //set tag
        self.installButton.tag = ACTION_SHOW_INFO;
    }
    //otherwise
