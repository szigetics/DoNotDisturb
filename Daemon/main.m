...
/* VIEW (STREAM) LOG */
// log stream --level debug --predicate="subsystem='com.objective-see.donotdisturb'"
...
int main(int argc, const char * argv[])
{
    ...
    @autoreleasepool
    {
        ...
        if(YES != [preferences.preferences[PREF_IS_DISABLED] boolValue])
        {
            ...
            //dbg msg
            os_log_debug(logHandle, "monitoring for lid events");
        }
        //user (prev) disabled
        // just log this fact, and don't start it
        else {
            os_log_debug(logHandle, "user has disabled DoNotDisturb, so, not enabling");
        }
