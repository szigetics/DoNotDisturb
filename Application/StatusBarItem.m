...
-(void)handler:(id)sender
{
    ...
    switch(((NSMenuItem*)sender).tag)
    {
        ...
        //prefs
        case prefs:
            [((AppDelegate*)NSApplication.sharedApplication.delegate) showPreferences:TOOLBAR_MODES];
            break;
        
        //telegram
        case telegram:
            [((AppDelegate*)NSApplication.sharedApplication.delegate) showPreferences:TOOLBAR_ALERTS];
            break;
        ...
        case uninstall:
        {
            ...
            //init path
            installer = [NSBundle.mainBundle URLForResource:@"DoNotDisturb Installer" withExtension:@".app"];
...
-(void)setState
{
    ...
    if(self.isDisabled)
    {
        ...
        [self.statusItem.menu itemWithTag:status].title = @"DoNotDisturb: Disabled";
        [self.statusItem.menu itemWithTag:toggle].title = @"Enable";
    }
    else
    {
        self.statusItem.button.alphaValue = 1.0;
        [self.statusItem.menu itemWithTag:status].title = @"DoNotDisturb: Enabled";
